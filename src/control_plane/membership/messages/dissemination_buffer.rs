...
pub(in crate::control_plane::membership) const MAX_GOSSIP_BYTES: usize = 1400;
...
impl Disseminable for SwimNode {
    type Key = crate::control_plane::NodeId;
    fn key(&self) -> Self::Key {
        self.node_id.clone()
    }
...
pub(in crate::control_plane::membership) struct DisseminationBuffer<T: Disseminable> {
...
impl<T: Disseminable> DisseminationBuffer<T> {
    pub(in crate::control_plane::membership) fn with_capacity(max_entries: usize) -> Self {
        Self {
            entries: Vec::new(),
            max_entries,
        }
    ...
    pub(in crate::control_plane::membership) fn collect(&mut self, budget: usize) -> Vec<T> {
...
#[inline]
pub(in crate::control_plane::membership) fn dissemination_count(cluster_size: usize) -> u32 {
    if cluster_size <= 1 {
        return LAMBDA;
    }
