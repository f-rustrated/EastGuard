...
impl RangeMerged {
    pub fn into_commands(self, shard_group_id: ShardGroupId) -> Vec<DataTransportCommand> {
        ...
        let commands = vec![DataTransportCommand::send_to_targets(
            vec![target],
            PlaceSegment {
                segment_key: self.segment_key,
                shard_group_id,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SegmentSealCommitted {
    pub segment_key: SegmentKey,
    pub replica_set: Replicas,
}

impl SegmentSealCommitted {
...
pub enum MetadataEvent {
    ...
    SegmentSealCommitted(SegmentSealCommitted),
