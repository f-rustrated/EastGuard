...
Application state machine for DS-RSM. One instance per shard group, owned by `Raft`. Manages topic, range, and segment metadata. Pure synchronous, no I/O. For the entity model and lifecycle, see `docs/metadata-management/d1_entity_model_and_lifecycle.md`.
