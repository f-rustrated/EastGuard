...
- Log storage backed by shared RocksDB instance, keyed by `(shard_group_id, key_type, index)`. See `docs/metadata-management/d2_replicated_apply_and_durability.md` § "Shared storage, isolated key ranges" for the design rationale.
...
5. **`last_included_index ≤ last_applied_index ≤ commit_index ≤ stabled_index ≤ log_last_index`** at all times. `last_included_index` is the durable snapshot boundary. `log_last_index` is logical: it equals the final retained suffix index, or the snapshot boundary when the suffix is empty. Retained entries are contiguous from `last_included_index + 1`; the boundary's persisted term supplies `term_at(last_included_index)` after its log entry is compacted.
