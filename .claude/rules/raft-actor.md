...
6. When an active segment loses exactly one node (whether it's the main leader or just a follower), the system pauses to figure out exactly where the data left off by asking the surviving nodes, rather than blindly closing the current data file and opening a new one.

- **Poll the Survivors:** If exactly one node dies, the system asks all remaining healthy nodes how much data they successfully saved.
- **Find the Minimum:** It looks at all the answers and picks the lowest number (the `min`). Because a successful save requires all nodes to agree, taking the minimum guarantees no confirmed data is lost.
- **Route Replies Directly:** Surviving nodes send their answers directly back to the specific coordinator that asked. This prevents replies from getting lost or mixed up if the system is in the middle of electing a new leader.
- **Open the Next File:** The system officially closes the old file at that exact `min` point and opens a new one right after it (`min+1`). This keeps the data continuous and easy to repair.
